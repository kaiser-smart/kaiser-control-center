...
assert.match(triageRenderSource, /dataBoxPlusReplyOverlay\(\)/);
assert.match(triageRenderSource, /data-ds-plus-access-open/);
assert.match(triageRenderSource, /dataBoxPlusAccessSettingsOverlay\(user\)/);
...
assert.match(triageRenderSource, /sentHistoryOnly \? "" : `<p class="ds-plus-triage-row__recommendation"/);
assert.match(triageRenderSource, /sentHistoryOnly \? "" : dataBoxPlusSummary\(message\)/);
assert.match(triageRenderSource, /sentHistoryOnly \? "" : facts/);
...
assert.match(appSource, /restoreDataBoxPlusInputFocus\(`\[data-ds-plus-reply-text=/);
...
assert.match(stylesSource, /\.ds-plus-detail\.ds-plus-access-dialog\s*\{[\s\S]*height: 100dvh/);
