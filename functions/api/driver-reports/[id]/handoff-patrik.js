...
function errorResponse(error) {
  if (error instanceof DriverPartRequestsStoreError) {
    return json({ error: error.message, apiStatus: "waiting", code: error.code, missingEndpoint: "POST /api/driver-reports/:id/handoff-patrik" }, error.status);
  }
  console.error("driver_reports.handoff_failed", { message: error?.message });
  return json({ error: "Předání Patrikovi se nepodařilo.", apiStatus: "waiting" }, 500);
}
