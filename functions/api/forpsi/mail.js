...
const messages={
  ...
  DRAFT_COPIES_DISABLED:'Vytváření upravených kopií konceptů ještě není zapnuté.',
  ...
  DRAFT_COPY_UNCERTAIN:'Výsledek kopírování není jistý. Zkontrolujte Koncepty ve Forpsi; stejný pokus neopakujte.',
...
export async function forwardForpsiMail({request,env}) {
  ...
  try {
    ...
    if(!command || Object.keys(command).some(k=>!['operation','payload'].includes(k)) ||
      !['list_mailboxes','list_folders','search_messages','read_message','composition_context','create_draft','open_draft','copy_draft','replace_draft'].includes(command.operation) || !command.payload || Array.isArray(command.payload) || typeof command.payload!=='object')throw new Error();
    if(!['create_draft','copy_draft','replace_draft'].includes(command.operation) && new TextEncoder().encode(JSON.stringify(command)).length>16384)return json({error:'Příliš velký požadavek.'},413);
  ...
  try {
    ...
    if(stillActive?.id!==user.id) return json({error:['create_draft','copy_draft','replace_draft'].includes(command.operation)?'Přístup již není aktivní. Koncept mohl být uložený; po obnovení přístupu ověřte složku Koncepty ve Forpsi.':'Účet již není aktivní nebo přihlášení vypršelo.',code:'AUTH_REQUIRED'},401);
