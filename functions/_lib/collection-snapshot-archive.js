...
const SOURCE_TABLE = "collection_import_rows";
...
async function latestCursor(archiveDb) {
  const row = await archiveDb.prepare(`
    ...
    LIMIT 1
  `).bind(SOURCE_DOMAIN, SOURCE_TABLE).first();
...
async function selectEligibleRows(sourceDb, cursor, cutoffIso, batchSize) {
...
async function recordAudit(auditDb, run) {
...
export async function archiveCollectionSnapshotChunk(env, options = {}) {
  ...
  try {
    const cursor = await latestCursor(archiveDb);
    const rows = await selectEligibleRows(sourceDb, cursor, cutoffIso, batchSize);
    ...
    if (!rows.length) {
      ...
      return { status: "completed", selectedRows: 0, transferredRows: 0, deletedRows: 0, cutoffIso };
    }
    ...
    const archiveBatchId = `collection-import-${sourceChecksum.slice(0, 24)}`;
    const objectKey = `collection-import-rows/${first.created_at.slice(0, 10)}/${archiveBatchId}.ndjson`;
...
export const __test = { MAX_BATCH_SIZE, DEFAULT_BATCH_SIZE, bytesToHex };
