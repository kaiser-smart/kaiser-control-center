...
export async function runRcsSmsAutopilotRetry(env, options = {}) {
  const db = env?.SMART_ODPADY_DB;
  if (!db) throw new Error("RCS/SMS Autopilot runner nemá D1 binding SMART_ODPADY_DB.");
...
export async function rcsSmsAutopilotStatus(env) {
  ...
  return {
    ...
    outboundEffects: mode === "live" && asyncActive ? "enabled_with_server_gates" : "disabled",
    ...operational
  };
}
