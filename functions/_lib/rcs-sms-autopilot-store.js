...
function database(env, required = false) {
  ...
  if (!db && required) {
    throw new RcsSmsAutopilotStoreError(
      "Databáze RCS/SMS Autopilota není nastavená. Chybí D1 binding SMART_ODPADY_DB.",
      503,
...
function dbError(error) {
  ...
  if (/no such table|no such column.*recipient_phone|rcs_sms_|rcs_message_dispatches/i.test(message)) {
    return new RcsSmsAutopilotStoreError(
      "Tabulky RCS/SMS Autopilota nejsou v D1 připravené. Spusťte migraci 0063_create_rcs_sms_autopilot.sql.",
      503,
...
async function resolveIdentity(db, phone) {
  const [users, customers, optOut] = await Promise.all([
    ...
    safeFirst(db, "SELECT id FROM customer_message_opt_out WHERE phone = ? LIMIT 1", phone)
  ]);
  ...
  if (matchedUsers.length === 1 && matchedCustomers.length === 0) {
    ...
    const employee = await safeFirst(
      db,
...
export async function rcsSmsAutopilotOperationalStatus(env) {
  const db = database(env, true);
  try {
    const [conversationCounts, messageCounts, requestCounts, lastEvent] = await Promise.all([
      db.prepare(`
        SELECT
          COUNT(*) AS total,
