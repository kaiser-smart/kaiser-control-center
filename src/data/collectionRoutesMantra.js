export const COLLECTION_ROUTES_MANTRA = Object.freeze({
  ...
  highlights: [
    ...
    },
    {
      ...
      text: "Před startem backend znovu ověří přihlášeného řidiče, datum a vozidlo vlastní trasy. Příprava, intro gong i agentův úvod běží pouze v hologramu bez mikrofonu. Po jediné závěrečné otázce KSO v témže hologramu samo zapne pětisekundový poslech bez velkého mikrofonního panelu; bez řeči zazní outro gong a hologram se zavře. Velký mikrofon slouží jen pro ruční vyvolání hovoru. Každý provozní zápis dál vyžaduje fyzické potvrzení."
    }
  ],
  sources: [
...
- T-Cars: aktuální poloha, dostupnost vozidla a poslední ověřený stav nádrže; jednotku, kterou API neposkytlo, nevymýšlej.
...
Hlasová Šarlota je samostatná přátelská konverzační asistentka. Po fyzickém potvrzení dnešní trasy nesmí přečíst pevnou backendovou šablonu. Backend předá pouze technický marker a strukturovaný ověřený kontext; KSO technickou First Message umlčí a vyžádá si skutečné úvodní hlášení od aktivního ElevenLabs agenta. Slyšitelné přivítání proto vytváří LLM podle skutečně aktivního Promptu, připojené Knowledge Base, Tools a dynamic variables modulu Svozové trasy. V pořadí použije ověřený vokativ, počet a první stanoviště, čerstvé počasí, poslední ověřený stav nádrže T-Cars bez domyšlené jednotky a nepřítomné dispečery bez soukromého důvodu; na potvrzení trasy se znovu neptá. Chybějící údaj vynechá. Automatický úvod skončí právě jednou otázkou, zda řidič potřebuje něco upřesnit, a až do jejího dohrání vůbec nezapíná mikrofon. Potom ve stejné holografické relaci pět sekund poslouchá. První řeč řidiče limit zruší a rozhovor pokračuje libovolně dlouho; bez řeči zazní outro gong a hologram se zavře.
...
Dispečerská hlášení mají samostatnou horní záložku Hlášení. Výchozí je vždy PROVOZ; TEST je výrazně označený a používá pouze izolovaná data. Stavový tok je Nové, Převzato, Řeší se a Vyřešeno. Uzavření vyžaduje výsledek řešení. Nevyřešený případ zůstává ve stavu Řeší se a má důvod, další krok, odpovědnou osobu a termín. E-mail a SMS se evidují odděleně a stav Doručeno vzniká pouze z platného provider webhooku. Dokud ostré providery a webhooky nejsou ověřené, PROVOZ nic neodesílá; TEST používá bezpečný adaptér bez kontaktu skutečného zákazníka.
