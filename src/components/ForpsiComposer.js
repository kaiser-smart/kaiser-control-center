...
let state={owner:null,mailboxId:'',epoch:0,root:null,api:null,guard:null,context:null,form:null,source:null,onSaved:null,dirty:false,busy:false,blocked:false,error:'',notice:'',submitted:null,preview:false};
...
export function discardForpsiComposer(){state.form=null;state.source=null;state.dirty=false;state.submitted=null;state.blocked=false;state.preview=false;state.error='';paint();}
...
function paint(){
  ...
  state.root.innerHTML=`<div class="forpsi-actions">${!d?button('open','Nový koncept'):''}</div>
    ...
    ${d&&c?`<section class="forpsi-card forpsi-composer"><h2>${copy?'Upravená kopie konceptu':state.source?'Upravit koncept':'Nový koncept'}</h2><p>Od: <strong>${escape((state.source?.senderName??c.profile.senderName)?`${state.source?.senderName??c.profile.senderName} <${c.address}>`:c.address)}</strong></p>
      ...
      ${state.preview?`<article class="forpsi-compose-preview"><h3>Náhled zprávy</h3><p>Komu: ${escape(d.to)}${d.cc?`<br>Kopie: ${escape(d.cc)}`:''}${d.bcc?`<br>Skrytá kopie: ${escape(d.bcc)}`:''}</p><h4>${escape(d.subject || '(bez předmětu)')}</h4><pre>${escape(state.source?d.text:composedText(d,c.profile))}</pre></article>`:''}</section>`:''}`;
}
async function context(open=false){
  ...
  try {
    ...
    else if(open){state.source=null;state.form={requestId:crypto.randomUUID(),to:'',cc:'',bcc:'',subject:'',text:'',useSignature:true};state.dirty=false;state.submitted=null;state.blocked=false;state.preview=false;state.notice='';}
...
export async function openForpsiDraft(reference,{copy=false}={}){
  ...
  finally{if(epoch===state.epoch){state.busy=false;paint();state.root?.querySelector('[name="to"]')?.focus?.();state.root?.querySelector('.forpsi-composer')?.scrollIntoView?.({block:'start'});}}
}
export async function saveForpsiComposer(){
  if(state.busy || !state.form || state.blocked)return false;
  const form=state.root?.querySelector('[data-composer-form]');if(!form?.reportValidity())return false;
  const epoch=state.epoch;
  const payload=state.submitted || (state.source?{
    mailboxId:state.mailboxId,requestId:state.form.requestId,reference:state.source.reference,
    expectedEtag:state.source.etag,message:composerPayload(state.mailboxId,state.form,0).message
  }:composerPayload(state.mailboxId,state.form,state.context.profile.revision));
...
export function mountForpsiComposer(root,{owner,mailboxId,apiJson,guard,onSaved}){
  if(owner!==state.owner || mailboxId!==state.mailboxId){state={owner,mailboxId,epoch:state.epoch+1,root:null,api:null,guard:null,context:null,form:null,source:null,onSaved:null,dirty:false,busy:false,blocked:false,error:'',notice:'',submitted:null,preview:false};}
  state.root=root;state.api=apiJson;state.onSaved=onSaved;state.guard=action=>state.dirty?guard(action):action();
  if(!root)return;
