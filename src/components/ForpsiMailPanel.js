...
let state={owner:null,epoch:0,root:null,api:null,loaded:false,busy:false,mailboxes:[],mailboxId:'',folders:[],draftFolder:null,replaceCapability:null,canEditDrafts:false,filters:emptyFilters(),applied:null,result:null,message:null,error:'',mode:null};
...
function paint(){
  ...
  root.innerHTML=`<div class="users-panel__head"><div><h1>Pošta</h1><p>Vaše přístupné schránky Forpsi.</p></div>${button('refresh','Obnovit přístup')}</div>
    ...
    ${state.mailboxId&&state.replaceCapability!==null?`<p class="forpsi-mail-status">Úprava existujících konceptů: ${state.replaceCapability?'server podporuje bezpečné nahrazení':'server bezpečné nahrazení nehlásí'}. ${state.canEditDrafts?'Dostupná pro tuto schránku.':'V SO.ai pro tuto schránku zatím nedostupná.'}</p>`:''}
...
async function refresh(){clearContent();state.loaded=false;state.mailboxes=[];state.folders=[];state.draftFolder=null;state.replaceCapability=null;state.canEditDrafts=false;state.mailboxId='';await request('list_mailboxes',{},data=>{state.mailboxes=data.mailboxes;state.loaded=true;});}
async function chooseMailbox(id){clearContent();state.mailboxId=id;state.folders=[];state.draftFolder=null;state.replaceCapability=null;state.canEditDrafts=false;state.filters=emptyFilters();if(!id){paint();return;}
  await request('list_folders',{mailboxId:id},data=>{state.folders=data.folders.filter(f=>f.selectable!==false);state.draftFolder=data.draftFolder;state.replaceCapability=typeof data.supportsReplace==='boolean'?data.supportsReplace:null;state.canEditDrafts=data.draftEditsEnabled===true&&data.supportsReplace===true&&data.draftWriteAllowed===true&&!!data.draftFolder;state.filters.folder=state.folders.some(f=>f.path==='INBOX')?'INBOX':state.folders[0]?.path || '';});}
...
export function mountForpsiMail(app,{apiJson,owner,guard}){
  ...
  if(state.owner!==owner || !root){state={owner,epoch:state.epoch+1,root:null,api:apiJson,loaded:false,busy:false,mailboxes:[],mailboxId:'',folders:[],draftFolder:null,replaceCapability:null,canEditDrafts:false,filters:emptyFilters(),applied:null,result:null,message:null,error:'',mode:null};}
