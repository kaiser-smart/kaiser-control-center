...
## KB ADAA onboarding pro pohledávky

Import pohledávek obsahuje read-only kontrolu připravenosti Account Direct Access API pro Komerční banku:

```text
GET /api/receivables/kb/onboarding
```

Endpoint nevolá banku, nezobrazuje hodnoty secretů, neukládá bankovní transakce a nevytváří platební příkazy. Pouze vrací stav serverových proměnných `KB_ADAA_*`, doporučený další krok a bezpečnostní pojistky pro budoucí read-only probe.
